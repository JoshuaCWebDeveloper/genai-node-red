...
// uibuilder custom
const uiblib = require('../lib/uiblib')  // Utility library for uibuilder
const tilib = require('../lib/tilib')   // General purpose library (by Totally Information)
const packageMgt = require('../lib/package-mgt')
const tiEvents = require('@totallyinformation/ti-common-event-handler') // https://github.com/EventEmitter2/EventEmitter2
const fslib  = require('../lib/fs')   // File/folder handling library (by Totally Information)
...
try {
    ...
    var sockets = require('../lib/socket') // eslint-disable-line no-var
} catch (e) {
    console.error('[uibuilder] REQUIRE SOCKET failed::', e)
}
try {
    // Singleton, only 1 instance of this class will ever exist. So it can be used in other modules within Node-RED.
    var web = require('../lib/web') // eslint-disable-line no-var
} catch (e) {
...
const uib = {
    ...
    moduleName: 'ui-builder',
    feModuleName: 'uibuilder',
    nodeRoot: '',
    deployments: {},
    instances: {},
    apps: {},
...
function nodeInstance(config) {
    ...
    this.url             = RED.util.evaluateNodeProperty(config.url, config.urlType, this) // Undefined or '' is not valid
