...
export const createRedTabs = (RED, $) => {
    ...
    function createTabs(options) {
        ...
        if (options.addButton) {
            ...
            var addButton = $(
                '<div class="red-ui-tab-button red-ui-tabs-add"><a href="#"><i class="fa fa-plus"></i></a></div>'
            ).appendTo(wrapper);
        ...
        if (options.searchButton) {
            ...
            var searchButton = $(
                '<div class="red-ui-tab-button red-ui-tabs-search"><a href="#"><i class="fa fa-list-ul"></i></a></div>'
            ).appendTo(wrapper);
        ...
        if (options.menu) {
            ...
            var menuButton = $(
                '<div class="red-ui-tab-button red-ui-tabs-menu"><a href="#"><i class="fa fa-caret-down"></i></a></div>'
            ).appendTo(wrapper);
        ...
        if (options.scrollable) {
            ...
            scrollLeft = $(
                '<div class="red-ui-tab-button red-ui-tab-scroll red-ui-tab-scroll-left"><a href="#" style="display:none;"><i class="fa fa-caret-left"></i></a></div>'
            )
                .appendTo(wrapper)
                .find('a');
            scrollLeft
            ...
            scrollRight = $(
                '<div class="red-ui-tab-button red-ui-tab-scroll red-ui-tab-scroll-right"><a href="#" style="display:none;"><i class="fa fa-caret-right"></i></a></div>'
            )
                .appendTo(wrapper)
                .find('a');
            scrollRight
        ...
        if (options.collapsible) {
            ...
            var collapsedButtonsRow = $(
                '<div class="red-ui-tab-link-buttons"></div>'
            ).appendTo(wrapper);
        ...
        function getSelection() {
            ...
            selection.each(function () {
                selectedTabs.push(
                    tabs[$(this).find('a').attr('href').slice(1)]
                );
            });
            return selectedTabs;
        }
                ...
                if (evt.metaKey || evt.ctrlKey) {
                    if (thisTab.hasClass('selected')) {
                        ...
                        if (!currentTab.hasClass('selected')) {
                            var currentTabObj =
                                tabs[
                                    currentTab.find('a').attr('href').slice(1)
                                ];
        ...
        function updateTabWidths() {
            ...
            if (options.collapsible) {
                ...
                var visibleCount =
                    collapsedButtonsRow.children(':visible').length;
                tabWidth = width - collapsedButtonsRow.width() - 10;
                var maxTabWidth = 198;
                var minTabWidth = 120;
                if (
                    tabWidth <= minTabWidth ||
                    (tabWidth < maxTabWidth && visibleCount > 5)
                ) {
                    ...
                    if (tabWidth <= minTabWidth || visibleCount > 6) {
                        //}!b.hasClass("red-ui-tab-link-button-pinned")) {
                        b.hide();
                    }
                    tabWidth = Math.max(
                        minTabWidth,
                        width - collapsedButtonsRow.width() - 10
                    );
                } else {
                ...
                if (options.scrollable) {
                    ...
                    if (tabWidth < options.minimumActiveTabWidth) {
                        ...
                        tabWidth =
                            (width -
                                12 -
                                options.minimumActiveTabWidth -
                                tabCount * 6) /
                            tabCount;
                        currentTabWidth = (100 * tabWidth) / width + '%';
                        currentActiveTabWidth =
                            options.minimumActiveTabWidth + 'px';
                    } else {
                        currentActiveTabWidth = 0;
                    }
                }
                ...
                if (tabWidth < 50) {
                    ...
                    ul.find('.red-ui-tab-label').css({
                        paddingLeft:
                            Math.min(12, Math.max(0, tabWidth - 38)) + 'px',
                    });
                } else {
        ...
        ul.find('li.red-ui-tab a')
            .on('mousedown', function (evt) {
                mousedownTab = evt.currentTarget;
            })
        ...
        var tabAPI = {
            addTab: function (tab, targetIndex) {
                ...
                if (targetIndex === 0) {
                    ...
                    li.insertAfter(
                        ul.find('li:nth-child(' + targetIndex + ')')
                    );
                } else {
                    li.appendTo(ul);
                }
                ...
                if (options.maximumTabWidth || tab.maximumTabWidth) {
                    li.css(
                        'maxWidth',
                        (options.maximumTabWidth || tab.maximumTabWidth) + 'px'
                    );
                }
                var link = $('<a/>', {
                    href: '#' + tab.id,
                ...
                if (tab.icon) {
                    ...
                    $('<i>', {
                        class: 'red-ui-tab-icon ' + tab.iconClass,
                    }).appendTo(link);
                }
                var span = $('<span/>', { class: 'red-ui-text-bidi-aware' })
                    .text(tab.label)
                    .appendTo(link);
                ...
                if (options.collapsible) {
                    ...
                    var pinnedLink = $(
                        '<a href="#' +
                            tab.id +
                            '" class="red-ui-tab-link-button"></a>'
                    );
                    if (tab.pinned) {
                        if (pinnedTabsCount === 0) {
                            pinnedLink.prependTo(collapsedButtonsRow);
                        } else {
                            pinnedLink.insertAfter(
                                collapsedButtonsRow.find(
                                    'a.red-ui-tab-link-button-pinned:last'
                                )
                            );
                        }
                    } else {
                        if (options.menu !== false) {
                            pinnedLink.insertBefore(
                                collapsedButtonsRow.find('a:last')
                            );
                        } else {
                    ...
                    if (tab.iconClass) {
                        ...
                        $('<i>', { class: defaultTabIcon }).appendTo(
                            pinnedLink
                        );
                    }
                    ...
                    if (options.onreorder) {
                        ...
                        pinnedLink.draggable({
                            ...
                            drag: function (event, ui) {
                                ui.position.left +=
                                    pinnedLinks[pinnedLinkIndex].left;
                                var tabCenter =
                                    ui.position.left +
                                    pinnedLinks[pinnedLinkIndex].width / 2;
                                for (var i = 0; i < pinnedLinks.length; i++) {
                                    if (
                                        i === pinnedLinkIndex ||
                                        pinnedLinks[i].menu ||
                                        pinnedLinks[i].el.is(':not(:visible)')
                                    ) {
                                        continue;
                                    }
                                    if (
                                        tabCenter > pinnedLinks[i].left &&
                                        tabCenter <
                                            pinnedLinks[i].left +
                                                pinnedLinks[i].width
                                    ) {
                                        if (i < pinnedLinkIndex) {
                                            pinnedLinks[i].left +=
                                                pinnedLinks[pinnedLinkIndex]
                                                    .width + 8;
                                            pinnedLinks[pinnedLinkIndex].el
                                                .detach()
                                                .insertBefore(
                                                    pinnedLinks[i].el
                                                );
                                            ...
                                            pinnedLinks[pinnedLinkIndex].el
                                                .detach()
                                                .insertAfter(pinnedLinks[i].el);
                                        }
                                        pinnedLinks[i].el.css({
                            ...
                            stop: function (event, ui) {
                                dragActive = false;
                                collapsedButtonsRow.children().css({
                                    position: 'relative',
                                    left: '',
                                    transition: '',
                                });
                ...
                if (options.onselect) {
                    $(
                        '<i class="red-ui-tabs-badge-selected fa fa-check-circle"></i>'
                    ).appendTo(badges);
                }

                // link.attr("title",tab.label);
                RED.popover.tooltip(link, function () {
                    return RED.utils.sanitize(tab.label);
                });
                ...
                if (options.onreorder && !options.collapsible) {
                    ...
                    li.draggable({
                        ...
                        drag: function (event, ui) {
                            ui.position.left +=
                                tabElements[tabDragIndex].left +
                                scrollContainer.scrollLeft();
                            var tabCenter =
                                ui.position.left +
                                tabElements[tabDragIndex].width / 2 -
                                scrollContainer.scrollLeft();
                            for (var i = 0; i < tabElements.length; i++) {
                                ...
                                if (
                                    ...
                                    if (i < tabDragIndex) {
                                        tabElements[i].left +=
                                            tabElements[tabDragIndex].width + 8;
                                        tabElements[tabDragIndex].el
                                            .detach()
                                            .insertBefore(tabElements[i].el);
                                    } else {
                                        tabElements[i].left -=
                                            tabElements[tabDragIndex].width + 8;
                                        tabElements[tabDragIndex].el
                                            .detach()
                                            .insertAfter(tabElements[i].el);
                                    }
                                    ...
                                    tabElements.splice(
                                        i,
                                        0,
                                        tabElements.splice(tabDragIndex, 1)[0]
                                    );
                        ...
                        stop: function (event, ui) {
                            ...
                            activateTab(
                                tabElements[tabDragIndex].el.data('tabId')
                            );
                        },
                    });
                }
            ...
            renameTab: function (id, label) {
                ...
                tab.find('span.red-ui-text-bidi-aware')
                    .text(label)
            ...
            order: function (order) {
                ...
                var existingTabOrder = $.makeArray(
                    ul.children().map(function () {
                        return $(this).data('tabId');
                    })
                );
                ...
                for (i = 0; i < order.length; i++) {
                    if (existingTabMap[order[i]]) {
                        ...
                        if (options.collapsible) {
                            pinnedButtons[order[i]].appendTo(
                                collapsedButtonsRow
                            );
                        }
