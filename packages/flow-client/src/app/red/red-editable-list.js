 ...
 **/
export const applyEditableList = function (RED, $) {
    /**
     * options:
    ...
    $.widget('nodered.editableList', {
        _create: function () {
            ...
            buttons.forEach(function (button) {
                var element = $(
                    '<button type="button" class="red-ui-button red-ui-button-small red-ui-editableList-addButton" style="margin-top: 4px; margin-right: 5px;"></button>'
                )
            ...
            if (this.options.sortable) {
                var handle =
                    typeof this.options.sortable === 'string'
        ...
        _resize: function () {
            ...
            if (this.options.resizeItem) {
                ...
                this.element.children().each(function (i) {
                    that.options.resizeItem(
                        $(this).children('.red-ui-editableList-item-content'),
                        i
                    );
                });
            }
            ...
            if (this.activeSort) {
                ...
                items.sort(function (A, B) {
                    return that.activeSort(
                        $(A)
                            .children('.red-ui-editableList-item-content')
                            .data('data'),
                        $(B)
                            .children('.red-ui-editableList-item-content')
                            .data('data')
        ...
        insertItemAt: function (data, index) {
            ...
            var li = $('<li>');
            var row = $('<div/>')
                .addClass('red-ui-editableList-item-content')
                .appendTo(li);
            row.data('data', data);
            ...
            if (this.options.removable) {
                var deleteButton = $('<a/>', {
                    href: '#',
                    class: 'red-ui-editableList-item-remove red-ui-button red-ui-button-small',
                }).appendTo(li);
        ...
        items: function () {
            return this.element.children().map(function (i) {
                return $(this).children('.red-ui-editableList-item-content');
            });
        },
        empty: function () {
            this.element.empty();
        ...
        show: function (item) {
            ...
            if (items.length > 0) {
                this.uiContainer.scrollTop(
                    this.uiContainer.scrollTop() + items.position().top
                );
            }
        },
        getItem: function (li) {
