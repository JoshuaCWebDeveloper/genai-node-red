import { createRedMonaco } from './red-monaco';
 ...
 */
export const createRedCodeEditor = (RED, $) => {
    const MONACO = 'monaco';
    const ACE = 'ace';
    ...
    function init() {
        ...
        try {
            ...
            if (
                !selectedCodeEditor ||
                (editorChoice === MONACO && (browser.ie || !window.monaco))
            ) {
            ...
            console.warn(
                "Problem initialising '" + editorChoice + "' code editor",
                error
            );
        }
        if (!initialised) {
        ...
        $(
            '<div id="red-ui-image-drop-target"><div data-i18n="[append]workspace.dropImageHere"><i class="fa fa-download"></i><br></div></div>'
    ...
    function create(options) {
        ...
        if (this.editor.type === MONACO) {
            ...
            if (!options.element) {
                console.warn(
                    'createEditor() options.element or options.id is not valid',
                    options
                );
                $('#dialog-form').append(
                    '<div id="' + options.id + '" style="display: none;" />'
                );
            }
