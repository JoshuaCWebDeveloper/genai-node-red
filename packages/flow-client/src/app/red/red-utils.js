...
export const createRedUtils = () => {
    ...
    const descriptionList = {
        ...
        renderer(token) {
            return `<dl class="message-properties">${this.parser.parseInline(
                token.tokens
    ...
    const description = {
        ...
        renderer(token) {
            return `\n<dt>${this.parser.parseInline(
                token.dt
            )}<span class="property-type">${this.parser.parseInline(
                token.types
            )}</span></dt><dd>${this.parser.parseInline(token.dd)}</dd>`;
        },
    ...
    function buildMessageSummaryValue(value) {
        ...
        if (Array.isArray(value)) {
            result = $(
                '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-meta"></span>'
            ).text('array[' + value.length + ']');
        } else if (value === null) {
            result = $(
                '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-null">null</span>'
            );
        } else if (typeof value === 'object') {
            if (value.hasOwnProperty('type') && value.type === 'undefined') {
                result = $(
                    '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-null">undefined</span>'
                );
            } else if (
                ...
                value.hasOwnProperty('type') &&
                (value.type === 'number' || value.type === 'bigint')
            ) {
                result = $(
                    '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-number"></span>'
                ).text(value.data);
            } else if (
                value.hasOwnProperty('type') &&
                value.type === 'regexp'
                ...
                result = $(
                    '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-string"></span>'
                ).text(value.data);
            } else {
                result = $(
                    '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-meta">object</span>'
                );
            }
            ...
            result = $(
                '<span class="red-ui-debug-msg-object-value red-ui-debug-msg-type-other"></span>'
            ).text('' + value);
        }
        return result;
    }
    ...
    function addMessageControls(
        ...
        var tools = $('<span class="red-ui-debug-msg-tools"></span>').appendTo(
            obj
        );
        var copyTools = $(
            '<span class="red-ui-debug-msg-tools-copy button-group"></span>'
        ).appendTo(tools);
        if (!!key) {
            var copyPath = $(
                '<button class="red-ui-button red-ui-button-small"><i class="fa fa-terminal"></i></button>'
            )
                .appendTo(copyTools)
                .on('click', function (e) {
                    e.preventDefault();
                    ...
                    RED.clipboard.copyText(
                        key,
                        copyPath,
                        'clipboard.copyMessagePath'
                    );
                });
            RED.popover.tooltip(
                copyPath,
                RED._('node-red:debug.sidebar.copyPath')
            );
        }
        var copyPayload = $(
            '<button class="red-ui-button red-ui-button-small"><i class="fa fa-clipboard"></i></button>'
        )
            .appendTo(copyTools)
            .on('click', function (e) {
                e.preventDefault();
                ...
                RED.clipboard.copyText(
                    msg,
                    copyPayload,
                    'clipboard.copyMessageValue'
                );
            });
        RED.popover.tooltip(
            copyPayload,
            RED._('node-red:debug.sidebar.copyPayload')
        );
        if (strippedKey !== undefined && strippedKey !== '') {
            var isPinned = pinnedPaths[sourceId].hasOwnProperty(strippedKey);

            var pinPath = $(
                '<button class="red-ui-button red-ui-button-small red-ui-debug-msg-tools-pin"><i class="fa fa-map-pin"></i></button>'
            )
                .appendTo(tools)
                .on('click', function (e) {
                    e.preventDefault();
                    ...
                    if (pinnedPaths[sourceId].hasOwnProperty(strippedKey)) {
                        ...
                        var rootedPath =
                            ...
                            (strippedKey[0] === '[' ? '' : '.') +
                            strippedKey;
                        pinnedPaths[sourceId][strippedKey] =
                            normalisePropertyExpression(rootedPath);
            ...
            RED.popover.tooltip(
                pinPath,
                RED._('node-red:debug.sidebar.pinPath')
            );
        }
        if (extraTools) {
            var t = extraTools;
    ...
    function formatBuffer(element, button, sourceId, path, cycle) {
        ...
        if (format === 'raw') {
            button.text('raw');
            element
                .removeClass('red-ui-debug-msg-buffer-string')
                .addClass('red-ui-debug-msg-buffer-raw');
        } else if (format === 'string') {
            button.text('string');
            element
                .addClass('red-ui-debug-msg-buffer-string')
                .removeClass('red-ui-debug-msg-buffer-raw');
        }
    }
    ...
    function createObjectElement(obj, options) {
        ...
        element.collapse = function () {
            element
                .find('.red-ui-debug-msg-expandable')
                .parent()
                .addClass('collapsed');
        };
        header = $('<span class="red-ui-debug-msg-row"></span>').appendTo(
            element
        );
        if (sourceId) {
            addMessageControls(
        ...
        if (!key) {
            ...
            if (sourceId) {
                ...
                element.clearPinned = function () {
                    element
                        .find('.red-ui-debug-msg-row-pinned')
                        .removeClass('red-ui-debug-msg-row-pinned');
                    pinnedPaths[sourceId] = {};
                };
            }
        } else {
            if (!hideKey) {
                $('<span class="red-ui-debug-msg-object-key"></span>')
                    .text(key)
                    .appendTo(header);
        ...
        entryObj = $(
            '<span class="red-ui-debug-msg-object-value"></span>'
        ).appendTo(header);
        ...
        if (
            ...
            ((obj.__enc__ && obj.type === 'set') ||
                (obj.__enc__ && obj.type === 'array') ||
                obj.type === 'Buffer')
        ) {
            isArray = true;
            isArrayObject = true;
        ...
        if (obj === null || obj === undefined) {
            $(
                '<span class="red-ui-debug-msg-type-null">' + obj + '</span>'
            ).appendTo(entryObj);
        } else if (obj.__enc__ && obj.type === 'undefined') {
            $(
                '<span class="red-ui-debug-msg-type-null">undefined</span>'
            ).appendTo(entryObj);
        } else if (
            obj.__enc__ &&
            (obj.type === 'number' || obj.type === 'bigint')
        ) {
            e = $(
                '<span class="red-ui-debug-msg-type-number red-ui-debug-msg-object-header"></span>'
            )
                .text(obj.data)
                .appendTo(entryObj);
        } else if (
            typeHint === 'regexp' ||
            (obj.__enc__ && obj.type === 'regexp')
        ) {
            e = $(
                '<span class="red-ui-debug-msg-type-string red-ui-debug-msg-object-header"></span>'
            )
                .text(typeof obj === 'string' ? obj : obj.data)
                .appendTo(entryObj);
        } else if (
            typeHint === 'function' ||
            ...
            e = $(
                '<span class="red-ui-debug-msg-type-meta red-ui-debug-msg-object-header"></span>'
            )
                .text('function')
                .appendTo(entryObj);
        } else if (
            ...
            if (/[\t\n\r]/.test(obj)) {
                ...
                $(
                    '<i class="fa fa-caret-right red-ui-debug-msg-object-handle"></i> '
                ).prependTo(header);
                makeExpandable(
                    header,
                    function () {
                        $(
                            '<span class="red-ui-debug-msg-type-meta red-ui-debug-msg-object-type-header"></span>'
                        )
                            .text(typeHint || 'string')
                            .appendTo(header);
                        var row = $(
                            '<div class="red-ui-debug-msg-object-entry collapsed"></div>'
                        ).appendTo(element);
                        $('<pre class="red-ui-debug-msg-type-string"></pre>')
                            .text(obj)
                            .appendTo(row);
                    },
                    function (state) {
            ...
            if (/^#[0-9a-f]{6}$/i.test(obj)) {
                $('<span class="red-ui-debug-msg-type-string-swatch"></span>')
                    .css('backgroundColor', obj)
                    .appendTo(e);
            }
        } else if (typeof obj === 'number') {
            e = $(
                '<span class="red-ui-debug-msg-type-number"></span>'
            ).appendTo(entryObj);
            ...
            if (originalLength > 0) {
                ...
                var arrayRows = $(
                    '<div class="red-ui-debug-msg-array-rows"></div>'
                ).appendTo(element);
                element.addClass('red-ui-debug-msg-buffer-raw');
            }
            if (key) {
                headerHead = $(
                    '<span class="red-ui-debug-msg-type-meta"></span>'
                )
                    .text(typeHint || type + '[' + originalLength + ']')
                    .appendTo(entryObj);
            } else {
                headerHead = $(
                    '<span class="red-ui-debug-msg-object-header"></span>'
                ).appendTo(entryObj);
                ...
                if (arrayLength === 0) {
                    $(
                        '<span class="red-ui-debug-msg-type-meta">empty</span>'
                    ).appendTo(headerHead);
                }
                $('<span> ]</span>').appendTo(headerHead);
            }
            if (originalLength > 0) {
                makeExpandable(
                    ...
                    function () {
                        ...
                        if (type === 'buffer') {
                            var stringRow = $(
                                '<div class="red-ui-debug-msg-string-rows"></div>'
                            ).appendTo(element);
                            var sr = $(
                                '<div class="red-ui-debug-msg-object-entry collapsed"></div>'
                            ).appendTo(stringRow);
                            ...
                            $(
                                ...
                                .appendTo(sr);
                            var bufferOpts = $(
                                '<span class="red-ui-debug-msg-buffer-opts"></span>'
                            ).appendTo(headerHead);
                            var switchFormat = $(
                                '<a class="red-ui-button red-ui-button-small" href="#"></a>'
                            )
                                .text('raw')
                                .appendTo(bufferOpts)
                                .on('click', function (e) {
                        ...
                        if (fullLength <= 10) {
                            for (i = 0; i < fullLength; i++) {
                                ...
                                subElements[path + '[' + i + ']'] =
                                    createObjectElement(data[i], {
                                        key: '' + i,
                                        typeHint:
                                            type === 'buffer' ? 'hex' : false,
                                        hideKey: false,
                                        path: path + '[' + i + ']',
                                        sourceId: sourceId,
                                        rootPath: rootPath,
                            ...
                            if (fullLength < originalLength) {
                                $(
                                    '<div class="red-ui-debug-msg-object-entry collapsed"><span class="red-ui-debug-msg-object-key">[' +
                                        fullLength +
                                        ' &hellip; ' +
                                        originalLength +
                                        ']</span></div>'
                    ...
                    function () {
                        ...
                        for (i = 0; i < keys.length; i++) {
                            ...
                            if (newPath !== undefined) {
                                if (
                                    /^[a-zA-Z_$][0-9a-zA-Z_$]*$/.test(keys[i])
                                ) {
                                    newPath +=
                                        (newPath.length > 0 ? '.' : '') +
                                        keys[i];
                        ...
                        if (keys.length === 0) {
                            $(
                                '<div class="red-ui-debug-msg-object-entry red-ui-debug-msg-type-meta collapsed"></div>'
                            )
                                .text('empty')
                                .appendTo(element);
                        }
            ...
            if (key) {
                ...
                for (i = 0; i < keysLength; i++) {
                    $('<span class="red-ui-debug-msg-object-key"></span>')
                        ...
                        .appendTo(headerHead);
                    $('<span>: </span>').appendTo(headerHead);
                    buildMessageSummaryValue(data[keys[i]]).appendTo(
                        headerHead
                    );
                    if (i < keysLength - 1) {
                        $('<span>, </span>').appendTo(headerHead);
            ...
            $('<span class="red-ui-debug-msg-type-other"></span>')
                .text('' + obj)
                .appendTo(entryObj);
        }
        if (exposeApi) {
    ...
    function normalisePropertyExpression(str, msg) {
        ...
        if (length === 0) {
            throw createError(
                'INVALID_EXPR',
                'Invalid property expression: zero-length'
            );
        }
        var parts = [];
        var start = 0;
                ...
                if (c === "'" || c === '"') {
                    ...
                    if (/^msg[.\[]/.test(str.substring(i + 1))) {
                        ...
                        for (var j = i + 1; j < length; j++) {
                            ...
                            if (depth === 0) {
                                try {
                                    ...
                                    throw createError(
                                        'INVALID_EXPR',
                                        'Invalid expression started at position ' +
                                            (i + 1)
                                    );
                                }
                            }
                        ...
                        if (depth > 0) {
                            throw createError(
                                'INVALID_EXPR',
                                "Invalid property expression: unmatched '[' at position " +
                                    i
                            );
                        }
                        continue;
                    ...
                    throw createError(
                        'INVALID_EXPR',
                        "Invalid property expression: unexpected ' ' at position " +
                            i
                    );
                }
            } else {
    ...
    function separateIconPath(icon) {
        ...
        if (icon) {
            ...
            if (index === 0) {
                icon = icon.substring(
                    (RED.settings.apiRootUrl + 'icons/').length
                );
            }
            var match = /^((?:@[^/]+\/)?[^/]+)\/(.*)$/.exec(icon);
            if (match) {
    ...
    function getNodeIcon(def, node) {
        ...
        if (node && node.type === '_selection_') {
            ...
            (node && /^_action_:/.test(node.type)) ||
            /^_action_:/.test(def.type)
        ) {
            ...
            if (isIconExists(iconPath)) {
                ...
                iconPath.module !== 'font-awesome' &&
                /.png$/i.test(iconPath.file)
            ) {
    ...
    function getNodeLabel(node, defaultLabel) {
        ...
        if (node.type === 'tab') {
            ...
            try {
                l =
                    (typeof l === 'function' ? l.call(node) : l) ||
                    defaultLabel;
            } catch (err) {
    ...
    function getPaletteLabel(nodeType, def) {
        ...
        if (typeof def.paletteLabel !== 'undefined') {
            try {
                ...
                console.log(
                    'Definition error: ' + nodeType + '.paletteLabel',
                    err
                );
            }
        }
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    if (themeRule.hasOwnProperty('category')) {
                        if (!themeRule.hasOwnProperty('_category')) {
                            themeRule._category = new RegExp(
                                themeRule.category
                            );
                        }
    ...
    function addSpinnerOverlay(container, contain) {
        var spinner = $(
            '<div class="red-ui-component-spinner "><img src="red/images/spin.svg"/></div>'
        ).appendTo(container);
        if (contain) {
    ...
    function decodeObject(payload, format) {
        if (format === 'number' && payload === 'NaN') {
            ...
            payload = JSON.parse(payload);
            payload =
                (payload.name ? payload.name + ': ' : '') + payload.message;
        } else if (format === 'null') {
            payload = null;
        ...
        if (iconPath.module === 'font-awesome') {
            ...
            if (fontAwesomeUnicode) {
                ...
                faIconElement.addClass(
                    'red-ui-palette-icon-fa fa fa-fw ' + faLarge + iconPath.file
                );
                return;
            }
            ...
            redIconElement.addClass(
                'red-ui-palette-icon red-ui-icons ' + iconPath.file
            );
            return;
        }
        var imageIconElement = $('<div/>', {
    ...
    function createNodeIcon(node, includeLabel) {
        ...
        if (includeLabel) {
            var labelText = RED.utils.getNodeLabel(
                node,
                node.name || node.type + ': ' + node.id
            );
            var label = $('<div>', { class: 'red-ui-node-label' }).appendTo(
                container
            );
            if (labelText) {
    ...
    function checkModuleAllowed(module, version, allowList, denyList) {
        ...
        if (allowedRule.wildcardPos !== deniedRule.wildcardPos) {
            ...
            return (
                allowedRule.module.toString().length >
                deniedRule.module.toString().length
            );
        }
        return false;
    }
    ...
    function getBrowserInfo() {
        ...
        try {
            ...
            r.mobile = /IEMobile|Windows Phone|Lumia/i.test(ua)
                ? 'w'
