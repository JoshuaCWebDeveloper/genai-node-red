...
export const createRedMonaco = function (RED, $) {
    ...
    const knownModules = {
        ...
        'fs/promises': {
            package: 'node',
            module: 'fs/promises',
            path: 'node/fs/promises.d.ts',
        },
        globals: {
            package: 'node',
            module: 'globals',
            path: 'node/globals.d.ts',
        },
        ...
        perf_hooks: {
            package: 'node',
            module: 'perf_hooks',
            path: 'node/perf_hooks.d.ts',
        },
        process: {
            package: 'node',
            module: 'process',
            path: 'node/process.d.ts',
        },
        querystring: {
            package: 'node',
            module: 'querystring',
            path: 'node/querystring.d.ts',
        },
        readline: {
            package: 'node',
            module: 'readline',
            path: 'node/readline.d.ts',
        },
        stream: { package: 'node', module: 'stream', path: 'node/stream.d.ts' },
        'stream/consumers': {
            package: 'node',
            module: 'stream/consumers',
            path: 'node/stream/consumers.d.ts',
        },
        'stream/promises': {
            package: 'node',
            module: 'stream/promises',
            path: 'node/stream/promises.d.ts',
        },
        'stream/web': {
            package: 'node',
            module: 'stream/web',
            path: 'node/stream/web.d.ts',
        },
        string_decoder: {
            package: 'node',
            module: 'string_decoder',
            path: 'node/string_decoder.d.ts',
        },
        ...
        'timers/promises': {
            ...
            path: 'node/timers/promises.d.ts',
        },
        tls: { package: 'node', module: 'tls', path: 'node/tls.d.ts' },
        trace_events: {
            package: 'node',
            module: 'trace_events',
            path: 'node/trace_events.d.ts',
        },
        ...
        worker_threads: {
            ...
            path: 'node/worker_threads.d.ts',
        },
        zlib: { package: 'node', module: 'zlib', path: 'node/zlib.d.ts' },
        'node-red': {
            package: 'node-red',
            module: 'node-red',
            path: 'node-red/index.d.ts',
        }, //only used if node-red types are concated by grunt.
        'node-red-util': {
            package: 'node-red',
            module: 'util',
            path: 'node-red/util.d.ts',
        },
        'node-red-func': {
            package: 'node-red',
            module: 'func',
            path: 'node-red/func.d.ts',
        },
    };
    const defaultServerSideTypes = [
        knownModules['node-red-util'],
        knownModules['node-red-func'],
        knownModules['globals'],
        knownModules['console'],
        knownModules['buffer'],
        knownModules['timers'],
        knownModules['util'],
    ];
    ...
    function init(options) {
        ...
        window.MonacoEnvironment.getWorker = function (workerId, label) {
            ...
            switch (label) {
                ...
                case 'less':
                    return getWorkerModule(
                        new URL(
                            '../../../../../node_modules/monaco-editor/esm/vs/language/css/css.worker?worker',
                            import.meta.url
                        ),
                        label
                    );
                ...
                case 'razor':
                    return getWorkerModule(
                        new URL(
                            '../../../../../node_modules/monaco-editor/esm/vs/language/html/html.worker?worker',
                            import.meta.url
                        ),
                        label
                    );
        ...
        try {
            const addTheme = function (themeThemeName, theme) {
                if (
                    (theme.rules && Array.isArray(theme.rules)) ||
                    theme.colors
                ) {
            ...
            if (editorOptions.theme) {
                if (
                    typeof editorOptions.theme == 'object' &&
                    RED.settings.editorTheme.theme
                ) {
                    let themeThemeName =
        ...
        function setupJSONata(_monaco) {
            ...
            _monaco.languages.setMonarchTokensProvider('jsonata', {
                ...
                keywords: [
                    ...
                    'NaN',
                    'undefined',
                ] /*.concat(Object.keys(jsonata.functions))*/,
                // keywords: [
            ...
            _monaco.languages.registerCompletionItemProvider('jsonata', {
                provideCompletionItems: function (model, position) {
                    ...
                    var jsonataSuggestions = jsonataFunctions.map(function (f) {
                        ...
                        var title = f + '(' + args + ')';
                        var body = RED._('jsonata:' + f + '.desc', {
                            defaultValue: '',
                        });
                        var insertText = (
                            jsonata.getFunctionSnippet(f) + ''
                        ).trim();
                        var documentation = {
                            value: '`' + title + '`\n\n' + body,
                        };
                        return createMonacoCompletionItem(
                            f,
                            insertText,
                            documentation,
                            range,
            ...
            _monaco.languages.registerHoverProvider('jsonata', {
                provideHover: function (model, position) {
                    ...
                    /** @type {monaco.Range} */
                    var r = new monaco.Range(
                        position.lineNumber,
                        position.column,
                        position.lineNumber,
                        position.column + w.word.length
                    );
                    return {
                        range: r,
                        contents: [
        ...
        function setupJSON(_monaco) {
            ...
            try {
                ...
                var diagnosticOptions = RED.settings.get(
                    'codeEditor.monaco.languages.json.jsonDefaults.diagnosticOptions'
                );
                var modeConfiguration = RED.settings.get(
                    'codeEditor.monaco.languages.json.jsonDefaults.modeConfiguration'
                );
                diagnosticOptions = Object.assign(
                    {},
                    diagnosticOptionsDefault,
                    diagnosticOptions || {}
                );
                ...
                if (modeConfiguration) {
                    _monaco.languages.json.jsonDefaults.setModeConfiguration(
                        modeConfiguration
                    );
                }
            } catch (error) {
        ...
        function setupHTML(_monaco) {
            ...
            try {
                var htmlDefaults = RED.settings.get(
                    'codeEditor.monaco.languages.html.htmlDefaults.options'
                );
                var handlebarDefaults = RED.settings.get(
                    'codeEditor.monaco.languages.html.handlebarDefaults.options'
                );
                if (htmlDefaults) {
                    _monaco.languages.html.htmlDefaults.setOptions(
                        htmlDefaults
                    );
                }
                if (handlebarDefaults) {
                    _monaco.languages.html.handlebarDefaults.setOptions(
                        handlebarDefaults
                    );
                }
            } catch (error) {
        ...
        function setupCSS(_monaco) {
            ...
            try {
                var cssDefaults_diagnosticsOption = RED.settings.get(
                    'codeEditor.monaco.languages.css.cssDefaults.diagnosticsOptions'
                );
                var lessDefaults_diagnosticsOption = RED.settings.get(
                    'codeEditor.monaco.languages.css.lessDefaults.diagnosticsOption'
                );
                var scssDefaults_diagnosticsOption = RED.settings.get(
                    'codeEditor.monaco.languages.css.scssDefaults.diagnosticsOption'
                );
                var cssDefaults_modeConfiguration = RED.settings.get(
                    'codeEditor.monaco.languages.css.cssDefaults.modeConfiguration'
                );
                var lessDefaults_modeConfiguration = RED.settings.get(
                    'codeEditor.monaco.languages.css.lessDefaults.modeConfiguration'
                );
                var scssDefaults_modeConfiguration = RED.settings.get(
                    'codeEditor.monaco.languages.css.scssDefaults.modeConfiguration'
                );
                if (cssDefaults_diagnosticsOption) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        cssDefaults_diagnosticsOption
                    );
                }
                if (lessDefaults_diagnosticsOption) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        lessDefaults_diagnosticsOption
                    );
                }
                if (scssDefaults_diagnosticsOption) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        scssDefaults_diagnosticsOption
                    );
                }
                if (cssDefaults_modeConfiguration) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        cssDefaults_modeConfiguration
                    );
                }
                if (lessDefaults_modeConfiguration) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        lessDefaults_modeConfiguration
                    );
                }
                if (scssDefaults_modeConfiguration) {
                    _monaco.languages.css.cssDefaults.setDiagnosticsOptions(
                        scssDefaults_modeConfiguration
                    );
                }
            } catch (error) {
                console.warn(
                    'monaco - Error setting up CSS/SCSS/LESS options',
                    error
                );
            }
        }
        ...
        function setupJS(_monaco) {
            var createJSSnippets = function (range) {
                return [
                    ...
                    createMonacoCompletionItem(
                        'dowhile',
                        'do {\n\t${2}\n} while (${1:condition});',
                        'Do-While Statement (JavaScript Language Basics)',
                        range
                    ),
                    createMonacoCompletionItem(
                        'while',
                        'while (${1:condition}) {\n\t${2}\n}',
                        'While Statement (JavaScript Language Basics)',
                        range
                    ),
                    createMonacoCompletionItem(
                        'switch',
                        'switch (${1:msg.topic}) {\n\tcase ${2:"value"}:\n\t\t${3}\n\t\tbreak;\n\tdefault:\n\t\t\n}',
                        'Switch Statement (JavaScript Language Basics)',
                        range
                    ),
                    createMonacoCompletionItem(
                        'trycatch',
                        'try {\n\t${2}\n} catch (${1:error}) {\n\t\n};',
                        'Try-Catch Statement (JavaScript Language Basics)',
                        range
                    ),
                    createMonacoCompletionItem(
                        'for (for loop)',
                    ...
                    createMonacoCompletionItem(
                        'node.status',
                        'node.status({fill:"${1|red,green,yellow,blue,grey|}",shape:"${2|ring,dot|}",text:"${3:message}"});',
                    ...
                    createMonacoCompletionItem(
                        'get (env)',
                        'env.get("${1|NR_NODE_ID,NR_NODE_NAME,NR_NODE_PATH,NR_GROUP_ID,NR_GROUP_NAME,NR_FLOW_ID,NR_FLOW_NAME|}");',
            ...
            try {
                ...
                var settingsComilerOptions =
                    RED.settings.get(
                        'codeEditor.monaco.languages.typescript.javascriptDefaults.compilerOptions'
                    ) || {};
                compilerOptions = Object.assign(
                    {},
                    compilerOptions,
                    settingsComilerOptions
                );
                ...
                var settingsDiagnosticsOptions =
                    RED.settings.get(
                        'codeEditor.monaco.languages.typescript.javascriptDefaults.diagnosticsOptions'
                    ) || {};
                diagnosticOptions = Object.assign(
                    {},
                    diagnosticOptions,
                    settingsDiagnosticsOptions
                );
                ...
                console.warn(
                    'monaco - Error setting javascriptDefaults',
                    error
                );
            }
        }
    ...
    function create(options) {
        ...
        if (!options.stateId && options.stateId !== false) {
            options.stateId = RED.editor.generateViewStateId(
                'monaco',
                options,
                (options.mode || options.title || '').split('/').pop()
            );
        }
        ...
        el = $('<div>')
            .appendTo(el)
            .addClass('red-ui-editor-text-container')[0];
        ...
        if (editorOptions.mouseWheelZoom == null) {
            editorOptions.mouseWheelZoom = true;
        }
        if (editorOptions.suggestFontSize == null) {
            editorOptions.suggestFontSize = 12;
        }
        if (editorOptions.formatOnPaste == null) {
            editorOptions.formatOnPaste = true;
        }
        if (editorOptions.foldingHighlight == null) {
            editorOptions.foldingHighlight = true;
        }
        if (editorOptions.foldStyle == null) {
            editorOptions.foldStyle = true;
        } //https://microsoft.github.io/monaco-editor/api/interfaces/monaco.editor.istandaloneeditorconstructionoptions.html#folding
        if (editorOptions.readOnly != null) {
            editorOptions.readOnly = editorOptions.readOnly;
        }
        if (editorOptions.lineNumbers === false) {
            editorOptions.lineNumbers = false;
        }
        if (editorOptions.theme == null) {
            editorOptions.theme = monacoThemes[0];
        }
        if (editorOptions.mode == null) {
            editorOptions.mode = convertAceModeToMonacoLang(options.mode);
        }
        if (editorOptions.automaticLayout == null) {
        ...
        editorOptions.roundedSelection =
            editorOptions.roundedSelection === false ? false : true; //default to true
        editorOptions.contextmenu =
            editorOptions.contextmenu === false ? false : true; //(context menu enable) default to true
        editorOptions.snippetSuggestions =
            editorOptions.enableSnippets === false ? false : true; //default to true //https://microsoft.github.io/monaco-editor/api/interfaces/monaco.editor.istandaloneeditorconstructionoptions.html#snippetsuggestions
        ...
        if (!editorOptions.wordSeparators) {
            if (
                editorOptions.language == 'jsonata' ||
        ...
        editorOptions.fixedOverflowWidgets =
            editorOptions.fixedOverflowWidgets === false ? false : true;
        ...
        if (options.clientSideSuggestions == null) {
            if (
                (options.mode + '').indexOf('nrjavascript') >= 0 ||
                (options.globals &&
                    (options.globals.RED || options.globals.Buffer))
            ) {
            ...
            if (!extraModuleLibs || extraModuleLibs.length == 0) {
                ...
                for (let index = 0; index < extraModuleLibs.length; index++) {
                    ...
                    if (km) {
                        ...
                        loadExtraModules[moduleName] = {
                            package: 'other',
                            module: moduleName,
                            path: 'other/' + moduleName + '.d.ts',
                        };
                    }
                }
                ...
                //         false,
                //         loadedLibs,
                //         function (err, extraLib) {
                //             loadList = loadList.filter(function (e) {
                //                 return e.module != extraLib.module;
                //             });
        ...
        function _executeEdits(editor, edits, endCursorState) {
            var isReadOnly = editor.getOption(
                monaco.editor.EditorOptions.readOnly.id
            );
            if (isReadOnly) {
                editor
                    .getModel()
        ...
        ed.getAnnotations = function getAnnotations() {
            ...
            try {
                ...
                if (_model !== null) {
                    ...
                    const modelMarkers =
                        monaco.editor.getModelMarkers(_model) || [];
                    const thisEditorsMarkers = modelMarkers.filter(function (
                        marker
                    ) {
                    ...
                    aceCompatibleMarkers = thisEditorsMarkers.map(function (
                        marker
                    ) {
                        return {
                            ...
                            type: monaco.MarkerSeverity[marker.severity]
                                ? monaco.MarkerSeverity[
                                      marker.severity
                                  ].toLowerCase()
                                : marker.severity, //ACE compatible
                        };
                    });
                }
            } catch (error) {
        ...
        function visibilityWatcher(element, callback) {
            try {
                var options = {
                    root:
                        $(element).closest('div.red-ui-tray-content')[0] ||
                        document,
                    attributes: true,
                    childList: true,
                };
        ...
        if (editorOptions.language === 'markdown') {
            ...
            ed.toolbar = RED.editor.customEditTypes['_markdown'].buildToolbar(
                toolbarRow,
                ed
            );
            if (options.expandable !== false) {
                var expandButton = $(
                    '<button type="button" class="red-ui-button" style="float: right;"><i class="fa fa-expand"></i></button>'
                ).appendTo(ed.toolbar);
                RED.popover.tooltip(
                    expandButton,
                    RED._('markdownEditor.expand')
                );
            ...
            var helpButton = $(
                '<button type="button" class="red-ui-editor-text-help red-ui-button red-ui-button-small"><i class="fa fa-question"></i></button>'
            ).appendTo($(el).parent());
            RED.popover.create({
                target: helpButton,
                trigger: 'click',
        ...
        ed.saveView = function (debuginfo) {
            if (!options.stateId) {
                return;
            } //only possible if created with a unique stateId
        ...
        ed.restoreView = function (state) {
            ...
            if (!_state) {
                return;
            } //no view state available
            try {
                if (ed.type) {
                    //is editor already initialised?
