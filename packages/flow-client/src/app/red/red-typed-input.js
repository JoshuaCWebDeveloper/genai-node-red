    ...
    */
    ...
    (function ($) {
        var contextParse = function (v, defaultStore) {
            var parts = RED.utils.parseContextKey(
                v,
            ...
            return function (val) {
                ...
                options.forEach(opt => {
                    ...
                    if (i > -1) {
                        ...
                        var el = $('<div/>', {
                            style: 'white-space:nowrap; overflow: hidden; flex-grow:1',
                        });
            ...
            node: {
                ...
                valueLabel: function (container, value) {
                    ...
                    var nodeLabel = $('<span>')
                        ...
                        .appendTo(container);
                    if (node) {
                        var colour = RED.utils.getNodeColor(
                            node.type,
                            node._def
                        );
                    ...
                    var eyeButton = $(
                        ...
                        .on('click', function (evt) {
                            ...
                            if (currentType === 'text') {
                                ...
                                setTimeout(function () {
                                    that.input.focus();
                                    that.input[0].setSelectionRange(
                                        cursorPosition,
                                        cursorPosition
                                    );
                                }, 50);
                        ...
                        .hide();
                    var eyeCon = $('<i class="fa fa-eye"></i>')
                        .css('margin-left', '-2px')
                        .appendTo(eyeButton);

                    if (value === '__PWRD__') {
                        var innerContainer = $(
                            '<div><i class="fa fa-asterisk"></i><i class="fa fa-asterisk"></i><i class="fa fa-asterisk"></i><i class="fa fa-asterisk"></i><i class="fa fa-asterisk"></i></div>'
                        )
                            .css({
                                padding: '6px 6px',
                                borderRadius: '4px',
                            })
                            .addClass('red-ui-typedInput-value-label-inactive')
                            .appendTo(container);
                        var editButton = $(
                            '<button type="button" class="red-ui-button red-ui-button-small"><i class="fa fa-pencil"></i></button>'
                        )
                            .appendTo(buttons)
                            .on('click', function (evt) {
                                evt.preventDefault();
                        ...
                        var cancelButton = $(
                            '<button type="button" class="red-ui-button red-ui-button-small"><i class="fa fa-times"></i></button>'
                        )
                            .css('margin-left', '3px')
                            .appendTo(buttons)
                            .on('click', function (evt) {
            ...
            if (!opt.multiple) {
                ...
                for (var i = 0; i < opt.options.length; i++) {
                    ...
                    if (currentValues.hasOwnProperty(val)) {
                        ...
                        selected.push(
                            typeof op === 'string' ? { value: op } : op.value
                        );
                    }
                }
        ...
        $.widget('nodered.typedInput', {
            _create: function () {
                ...
                try {
                    if (!nlsd && RED && RED._) {
                        ...
                        var contextOptions = contextStores
                            .map(function (store) {
                                return {
                                    value: store,
                                    label: store,
                            ...
                            .sort(function (A, B) {
                                if (A.value === RED.settings.context.default) {
                                    ...
                                    B.value === RED.settings.context.default
                                ) {
                                    return 1;
                                } else {
                    ...
                    this.input = $(
                        '<input class="red-ui-typedInput-input" type="text"></input>'
                    );
                    this.input.insertAfter(this.element);
                    this.input.val(this.element.val());
                    this.element.addClass('red-ui-typedInput');
                    this.uiWidth = this.element.outerWidth();
                    this.elementDiv = this.input
                        .wrap('<div>')
                        .parent()
                        .addClass('red-ui-typedInput-input-wrap');
                    this.uiSelect = this.elementDiv.wrap('<div>').parent();
                    var attrStyle = this.element.attr('style');
                    var m;
                    if (
                        (m = /width\s*:\s*(calc\s*\(.*\)|\d+(%|px))/i.exec(
                            attrStyle
                        )) !== null
                    ) {
                    ...
                    if (!this.options.types && this.options.type) {
                        ...
                        this.options.types =
                            this.options.types || Object.keys(allOptions);
                    }

                    this.selectTrigger = $(
                        '<button class="red-ui-typedInput-type-select" tabindex="0"></button>'
                    ).prependTo(this.uiSelect);
                    $('<i class="red-ui-typedInput-icon fa fa-caret-down"></i>')
                        .toggle(this.options.types.length > 1)
                    ...
                    this.selectTrigger
                        ...
                        .on('blur', function () {
                            ...
                            if (opt.hasValue === false) {
                                that.uiSelect.removeClass(
                                    'red-ui-typedInput-focus'
                                );
                            }
                        });
                    ...
                    this.optionSelectTrigger = $(
                        '<button tabindex="0" class="red-ui-typedInput-option-trigger" style="display:inline-block"><span class="red-ui-typedInput-option-caret"><i class="red-ui-typedInput-icon fa fa-caret-down"></i></span></button>'
                    ).appendTo(this.uiSelect);
                    ...
                    this.optionExpandButton = $(
                        '<button tabindex="0" class="red-ui-typedInput-option-expand" style="display:inline-block"></button>'
                    ).appendTo(this.uiSelect);
            ...
            _showOptionSelectMenu: function () {
                if (this.optionMenu) {
                    ...
                    var selectedOption = this.optionMenu.find(
                        "[value='" + targetValue + "']"
                    );
            ...
            _getLabelWidth: function (label, done) {
                ...
                if (labelWidth === 0) {
                    var wrapper = $('<div class="red-ui-editor"></div>')
                        .css({
                            position: 'absolute',
                            'white-space': 'nowrap',
                            top: -2000,
            ...
            _updateOptionSelectLabel: function (o) {
                ...
                if (this.typeMap[this.propertyType].valueLabel) {
                    if (opt.multiple) {
                        this.typeMap[this.propertyType].valueLabel.call(
                            this,
                            opt.hasValue
                                ? this.valueLabelContainer
                                : this.optionSelectLabel,
                    ...
                    if (!opt.multiple) {
                        if (o.icon) {
                            if (o.icon.indexOf('<') === 0) {
                                ...
                                // icon class
                                $('<i>', {
                                    class: 'red-ui-typedInput-icon ' + o.icon,
                                }).prependTo(this.optionSelectLabel);
                            }
                        } else if (o.label) {
            ...
            types: function (types) {
                ...
                if (this.typeList.length < 2) {
                    ...
                    this.selectTrigger.on(
                        'mousedown.red-ui-typedInput-focus-block',
                        function (evt) {
                            evt.preventDefault();
                        }
                    );
                    ...
                    this.selectTrigger.off(
                        'mousedown.red-ui-typedInput-focus-block'
                    );
                }
                this.selectTrigger.toggleClass(
                    'disabled',
                    this.typeList.length === 1
                );
                this.selectTrigger
                    .find('.fa-caret-down')
                    .toggle(this.typeList.length > 1);
                if (this.menu) {
                    this.menu.remove();
                }
                ...
                if (
                    this.typeList.length === 1 &&
                    !this.typeList[0].icon &&
                    (!this.typeList[0].label ||
                        this.typeList[0].showLabel === false)
                ) {
            ...
            type: function (type) {
                if (!arguments.length) {
                    ...
                    if (opt && this.propertyType !== type) {
                        ...
                        if (previousType && this.typeChanged) {
                            ...
                            if (
                                ...
                                if (this.oldValues.hasOwnProperty(opt.value)) {
                                    ...
                                    var validOptions = isOptionValueValid(
                                        opt,
                                        previousValue
                                    );
                                    if (this.options.debug) {
                        ...
                        if (opt.hasValue === false) {
                            this.selectTrigger.addClass(
                                'red-ui-typedInput-full-width'
                            );
                        } else {
                            this.selectTrigger.removeClass(
                                'red-ui-typedInput-full-width'
                            );
                        }
                        ...
                        if (opt.options) {
                            ...
                            if (this.optionSelectTrigger) {
                                ...
                                if (
                                    !that.activeOptions.hasOwnProperty(
                                        that.optionValue
                                    )
                                ) {
                                ...
                                if (!opt.hasValue) {
                                    ...
                                    var validValues = isOptionValueValid(
                                        opt,
                                        this.input.val()
                                    );
                                    if (!opt.multiple) {
                                        if (validValues) {
                                            that._updateOptionSelectLabel(
                                                validValues
                                            );
                                        } else {
                                            op = opt.options[0];
                                    ...
                                    if (opt.parse) {
                                        ...
                                        if (parts.option) {
                                            ...
                                            if (
                                                !this.activeOptions.hasOwnProperty(
                                                    selectedOption
                                                )
                                            ) {
                                                parts.option = Object.keys(
                                                    this.activeOptions
                                                )[0];
                                                selectedOption = parts.option;
                                            }
                                ...
                                this.optionMenu = this._createMenu(
                                    ...
                                    function (v) {
                                        if (!opt.multiple) {
                                            that._updateOptionSelectLabel(
                                                that.activeOptions[v]
                                            );
                                            if (!opt.hasValue) {
                                                that.value(
                                                    that.activeOptions[v].value
                                                );
                                            }
                                        } else {
                                ...
                                if (opt.expand) {
                                    if (opt.expand.icon) {
                                        ...
                                        this.optionExpandButtonIcon
                                            .removeClass()
                                            .addClass(
                                                'red-ui-typedInput-icon fa fa-ellipsis-h'
                                            );
                                    }
                                    this.optionExpandButton.shown = true;
                                    this.optionExpandButton.show();
                                    this.optionExpandButton.off('click');
                                    this.optionExpandButton.on(
                                        'click',
                                        function (evt) {
                                            evt.preventDefault();
                                            if (
                                                typeof opt.expand === 'function'
                                            ) {
                                                opt.expand.call(that);
                                            } else {
                                                var container = $('<div>');
                                                var content =
                                                    opt.expand.content.call(
                                                        that,
                                                        container
                                                    );
                                                var panel =
