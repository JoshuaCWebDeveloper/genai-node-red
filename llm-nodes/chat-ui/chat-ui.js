...
const { getSource } = require('../lib/uiblib')
...
const mod = {
    ...
    nodeName: 'chat-ui', // Note that 'uib-element' will be replaced with actual node-name. Do not forget to also add to package.json
}
...
function nodeInstance(config) {
    ...
    this.elementIdSource = config.elementId ?? '' // Swap to source naming
 ...
 * @returns {string} Error description or empty error string
 */
function buildChatUi(node, msg, parent) {
    const err = ''
    ...
    let chatHistory = [];
    if (msg && msg.chat && Array.isArray(msg.chat.history)) {
        chatHistory = msg.chat.history;
    }

    // Add a style tag to the parent
    parent.components.push({
        'type': 'style',
        'parent': node.parent,
        'slot': `            
    ...
    const chatHistoryDiv = {
        'type': 'div',
        'id': `${node.elementId}-chat-history`,
        'attributes': {
            'class': 'chat-history'
        },
        'components': [],
    ...
    frmBody.components.push(chatHistoryDiv)
    ...
    const formDiv = {
        ...
        'components': [
            {
                'type': 'textarea',
                'id': `${node.elementId}-next-message`,
                'attributes': {
